...
class Trapezoid extends Object3D {
    ...
    constructor(baseLength, topLength, high, width, color, material) {
        super(null,null,null,null, null);


        let plane = new Trapezoid2D(baseLength, topLength, high, color, material);
        plane.rotar(Math.PI/2,[1,0,0]);
        plane.trasladar([0, width/2, 0]);
        this.addChild(plane);

        plane = new Trapezoid2D(baseLength, topLength, high, color, material);
        plane.rotar(Math.PI,[0,1,0]);
        plane.rotar(Math.PI/2,[1,0,0]);
        plane.trasladar([0, width/2, 0]);
        this.addChild(plane);

        plane = new Plane(baseLength, width, color, material);
        plane.rotar(Math.PI, [1,0,0]);
        plane.trasladar([0,high/2,0]);
        this.addChild(plane);

        plane = new Plane(topLength, width, color, material);
        plane.trasladar([0,high/2,0]);
        this.addChild(plane);
        ...
        plane = new Plane(hypo, width, color, material);
